...
def deleteFiles(path, date, type=str):
    root_dir = Path(path)
    removed_files = []
    for subdir in root_dir.iterdir():
        if 'SEASON' in subdir.name:
            ...
            for episode in open_season.iterdir():
                if 'CREXXX' not in episode.name and type == 'exports':
                    final_dir = root_dir / subdir / episode / '1_EXPORTS' / '1_MASTERS' / '3_FACEBOOK'
                elif 'CREXXX' not in episode.name and type == 'preedits':
                    final_dir = root_dir / subdir / episode / '4_EXCHANGES' / '1_PRE-EDIT'
                try:
